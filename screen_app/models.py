...
class Product(models.Model):
    ...
    class Meta:
        ordering = ['code']  # Add default ordering
...
class ProductMedia(models.Model):
    ...
    file = models.FileField(
        ...
        validators=[FileExtensionValidator(allowed_extensions=['pdf', 'mp4', 'mov', 'zip', 'jpg', 'jpeg', 'png', 'gif'])]
    )
        ...
        if self.file.name.endswith('.zip'):
            with zipfile.ZipFile(self.file) as zip_file:
                for filename in zip_file.namelist():
                    ...
                    if extension.lower() in ['.pdf', '.mp4', '.mov', '.jpg', '.jpeg', '.png', '.gif']:
